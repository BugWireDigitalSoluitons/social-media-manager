...
User-selected MVP deployment: personal PC serves web/API on a fixed LAN port/IP, optionally named by router DNS. Use Clerk Hobby and authenticate from the beginning. Hobby is a billing plan, not a development/production environment choice. Exact development-key LAN IP/hostname behavior is unverified: first prove login/session refresh/logout/API access from a second device. Production Clerk requires configured domain/DNS and HTTPS; the prior private-DNS production proposal is not verified as supported. Development users do not transfer automatically to production; a subscription upgrade alone is not the migration plan. Outbound access to Clerk and Meta is needed. OAuth redirects and provider lifecycle callbacks have separate reachability requirements; resolve those before promising live connection behavior. Do not bypass authentication to make LAN access work.

The project is under OneDrive. Keep live PostgreSQL files, runtime .env secrets, and raw production dumps outside the synced source/vault tree on the deployment host. Choose that path during deployment rather than silently writing outside the project. User chose backups on this system only for MVP; off-system/cloud backup is deferred. Back up with database-aware tooling and test restores; local backups do not protect against host/disk loss, and live database file synchronization is not backup.
